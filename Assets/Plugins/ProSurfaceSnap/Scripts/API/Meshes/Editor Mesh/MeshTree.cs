...
namespace PSS
{
    public class MeshTree
    {
        ...
        public List<Vector3> OverlapModelVerts(OOBB modelOOBB)
        {
            ...
            HashSet<int> usedIndices = new HashSet<int>();
            var overlappedNodes = _tree.OverlapBox(modelOOBB);
            if (overlappedNodes.Count == 0) return new List<Vector3>();
            ...
            foreach (var node in overlappedNodes)
            {
                ...
                for (int ptIndex = 0; ptIndex < modelVerts.Length; ++ptIndex)
                {
                    ...
                    Vector3 modelVert = modelVerts[ptIndex];
                    if (BoxMath.ContainsPoint(modelVert, modelOOBB.Center, modelOOBB.Size, modelOOBB.Rotation))
                    {
